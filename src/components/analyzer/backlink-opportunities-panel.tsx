...
export function BacklinkOpportunitiesPanel({ slug, trackId }: BacklinkOpportunitiesPanelProps) {
    ...
    <div className="px-4 py-5">
      <div className="mb-3 flex items-center justify-between gap-3">
        <div className="flex items-center gap-2">
          ...
          <h4 className="text-sm font-medium text-foreground">Backlink Opportunities</h4>
          {rows.length > 0 && (
      ...
      {isEmpty ? (
        <div className="rounded-md border border-dashed border-border bg-muted/10 px-4 py-8 text-center">
          ...
          <p className="mt-1 text-xs text-muted-foreground">
            Generate a list of sites where you can earn a backlink relevant to this prompt. Takes
            10–30 seconds.
          </p>
          <Button
            variant="default"
...
function OpportunityRow({ opp }: { opp: BacklinkOpportunity }) {
  return (
    <li className="rounded-md border border-border bg-background px-3 py-2.5">
      ...
      {opp.description && <p className="mt-1 text-xs text-muted-foreground">{opp.description}</p>}
      {opp.rationale && (
